 ...
 * Does NOT require Redis/BullMQ — bypasses the queue entirely.
 * MongoDB is used only for deduplication; if unavailable, falls back to no dedup.
 *
...
async function main() {
  console.log("=== test-video-gen (Hacker News) ===\n");
  ...
  const { videoPath, durationSeconds } = await generateVideo(jobId, post.title, post.thumbnail);
